...
//    countQuery = "select * from (select id,name,birthday,customer_type_id as customerType, gender, id_card as idCard, phone_number as phoneNumber, email, address from customer where flag_remove = true) as a")
    @Query(value = "select c.id,c.name,c.birthday, ct.name as customerType, c.gender, c.id_card as idCard, c.phone_number as phoneNumber, c.email, c.address from customer c join customer_type ct on c.customer_type_id = ct.id where flag_remove = true",
            countQuery = "select *from (select c.id,c.name,c.birthday, ct.name as customerType, c.gender, c.id_card as idCard, c.phone_number as phoneNumber, c.email, c.address from customer c join customer_type ct on c.customer_type_id = ct.id where flag_remove = true) as a",
            nativeQuery = true)
